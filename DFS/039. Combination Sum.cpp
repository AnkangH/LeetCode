...
解析：组合问题。给定一个正整数数组array和一个正整数target，数组中的数字可以重复使用，求所有和为该正整数target的数组元素组合。
...
时间900+ms。解决方法是，对于每个当前选择的数字，向后进行dfs，这样对2进行dfs时，可以从237中选，而对3dfs时，只能从3和7中选，排除了重复
的组合。边界条件，当前值=目标值，和当前值大于目标值以及当前值小于目标值。方法一放上dfs+map删除重复元素的方法，是满足要求但很差的解决方案。
方法二是对每个数字，只对其后的数字dfs的解决方法，时间70ms，是方法一的10分之一。
*/
